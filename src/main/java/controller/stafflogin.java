...
public class stafflogin extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       // request.setAttribute("bean",bean);
        ...
        boolean rslt=con.checkstaffg1(username,password);
        if(rslt==true)
        {
        ...
        RequestDispatcher rs = request.getRequestDispatcher("dashboardstaff_01.jsp");
       ...
       try
        {
        dbconnect con = new dbconnect();
        boolean rslt=con.checkstaffg2(username,password);
        if(rslt==true)
        ...
        response.sendRedirect("dashboardstaff_02.jsp"); 
        }
        else
        {
        out.println("<p>incorrect details </p>");
